...
- **Map artwork**: `map-portrait-4k.png` at **2160×3840** pixels. This is the single source of truth; do not scale or crop it for calibration.
...
2. **Load the map**
   - The page loads `map-portrait-4k.png` from the same directory. Ensure the image is present and exactly 2160×3840.

3. **Add control points**
   - **Click** on the map at a known location (e.g. Newcastle upon Tyne). The tool computes the **image-space** pixel coordinates (x, y) in the 2160×3840 space and fills the X/Y fields (read-only).
...
- Coordinates are always in the **full 2160×3840** image space, even if the image is zoomed or scaled in the browser.
